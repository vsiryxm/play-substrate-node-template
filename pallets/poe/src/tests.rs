...
use frame_support::{assert_noop, assert_ok, BoundedVec}; // 引入BoundedVec
...
fn create_claim_works() {
    new_test_ext().execute_with(|| {
        ...
        // RuntimeOrigin::signed(1)，这里的1，是因为mock.rs文件中定义了type AccountId = u64;
...
fn create_claim_failed_when_claim_too_long() {
    new_test_ext().execute_with(|| {
        ...
        let claim = BoundedVec::<u8, <Test as Config>::MaxClaimLength>::try_from(vec![0, 1, 2]).unwrap(); //[0, 1]时刚刚好，[0, 1, 2]时就超出最大长度2了，见mock.rs中的配置
...
fn create_claim_failed_when_claim_already_exist() {
    new_test_ext().execute_with(|| {
        ...
        // assert_noop!()宏不会对链上存储进行任何修改
        assert_noop!(
            PoeModule::create_claim(RuntimeOrigin::signed(1), claim.clone()), // 再次创建同样的存证
...
fn revoke_claim_when_claim_not_exists() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PoeModule::revoke_claim(RuntimeOrigin::signed(1), claim.clone()), // 撤销不存在的存证
...
fn revoke_claim_when_not_claim_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PoeModule::revoke_claim(RuntimeOrigin::signed(2), claim.clone()), // 使用用户2撤销存证
...
fn transfer_claim_when_claim_not_exists() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PoeModule::transfer_claim(RuntimeOrigin::signed(1), claim.clone(), 2), // 用户1转移该存证给用户2
            Error::<Test>::ClaimNotExist
        );
...
fn transfer_claim_when_not_claim_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PoeModule::transfer_claim(RuntimeOrigin::signed(2), claim.clone(), 3), // 用户2转移该存证给用户3
            Error::<Test>::NotClaimOwner
        );
